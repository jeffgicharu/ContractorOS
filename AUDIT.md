# SDET Audit — ContractorOS (Step 01)

Baseline assessment performed before introducing SDET tooling. Captures stack, architecture, current testing/CI posture, and the gap inventory that subsequent steps will close.
...
- **DAST**: no OWASP ZAP run (manual or CI). Step 02 will introduce ZAP via the just-pulled `ghcr.io/zaproxy/zaproxy:stable` image.
...
- No published `pact-broker` URL. The `@pact-foundation/pact-cli` and `pact-broker` Rust CLI are now installed locally and ready to wire up.
...
## 8. Repo-Hygiene Notes (Out of Scope for Step 01 but Worth Capturing)
...
## 9. What Subsequent Steps Will Close

This audit's gap inventory drives the remaining 19 SDET sprint steps. The headline closures planned:

- **Step 02** — wire OWASP ZAP baseline + Trivy filesystem scan + `npm audit` / Snyk into CI.
- **Step 03** — Stryker mutation testing on `apps/api/src/modules/classification/scoring/` and on `packages/shared` Zod/state machines.
- **Step 04** — fix `pnpm test:integration`, scaffold `apps/api/test/integration/` with Supertest + transactional Postgres helper, cover the 12 controllers and 12 repositories.
- **Step 05** — k6 scripts for login, invoice list, classification re-score under realistic seed data; baseline numbers committed.
- **Step 06** — Pact consumer tests in `apps/web` against the shared Zod-derived schemas, provider verification in `apps/api`.
- **Steps 07-12** — Cypress (or Playwright) coverage for the unflowed flows in §7.1, plus `cypress-axe` accessibility gates and a Chromatic-style visual baseline.
- **Steps 13-17** — `apps/web` React Testing Library suites and `packages/shared` Zod/state-machine suites to hit the targets in `testing.md`.
- **Steps 18-20** — AI-assisted test-generation workflow docs, CI consolidation, and a portfolio-ready summary report.
